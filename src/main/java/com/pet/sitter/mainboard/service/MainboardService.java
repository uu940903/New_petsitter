...
public class MainBoardService {
    ...
    */
    ...
    public PetSitterDTO getDetail(Long no) {
        ...
        petSitterDTO.setMember(MemberDTO.builder().member(petsitterEntity.getMember()).build().toEntity());
...
//            String savePath = "/Users/사용자이름/springboot_img/" + storedFileName; // C:/springboot_img/9802398403948_내사진.jpg
   ...
   //수정 폼 요청시 호출 메서드
   public PetSitterDTO updateForm(Long no) {
        Petsitter petsitterEntity = petsitterRepository.findBySitterNo(no);
        PetSitterDTO petSitterDTO = PetSitterDTO.builder().petsitter(petsitterEntity).build();

        List<PetsitterFile> fileList = new ArrayList<>();
        fileList = petsitterFileRepository.findAllByPetsitter(petsitterEntity);
        petSitterDTO.setMember(MemberDTO.builder().member(petsitterEntity.getMember()).build().toEntity());
        ...
        if(!fileList.isEmpty()){
            List< PetsitterFile> petSitterFileDTOList = new ArrayList<>();
            for( PetsitterFile multipartFile : fileList){
                PetSitterFileDTO petSitterFileDTO = PetSitterFileDTO.builder().petsitterFile(multipartFile).build();
                petSitterFileDTOList.add(multipartFile);
            }
            PetSitterFileDTO petSitterFileDTO = new PetSitterFileDTO();
    ...
    /*//게시글 수정
    public void modify(WriteForm writeForm) {
        petsitter.setPetTitle(writeForm.getPetTitle());
        petsitter.setPetCategory(writeForm.getPetCategory());
        petsitter.setCategory(writeForm.getCategory());
        petsitter.setPrice(writeForm.getPrice());
        petsitter.setStartTime(writeForm.getStartTime());
        petsitter.setEndTime(writeForm.getEndTime());
        petsitter.setPetContent(writeForm.getPetContent());
        petsitter.setPetAddress(writeForm.getPetAddress());
        petsitterRepository.save(petsitter);
    }*/
    ...
    public void deletePetSitter (Petsitter petsitter) {
        petsitterRepository.deleteBySitterNo(petsitter.getSitterNo());
    }
